...
from app.models.requests import DownloadRequest, ListingRequest
...
class YtDlpDownloadService(DownloadService):
    def __init__(self, job_store: JobStore) -> None:
        ...
        self._listing_service = YtDlpListingService()
    ...
    def _resolve_instagram_profile_urls(
        ...
        if data.platform.lower() != "instagram":
            return None, None
        if not YtDlpListingService._is_instagram_profile_url(data.url):
            return None, None

        listing_request = ListingRequest(platform="instagram", url=data.url, limit=data.limit)
        items, error = self._listing_service.list_instagram_media(listing_request)
        if error:
            return None, error
        if not items:
    ...
    def queue_download(self, data: DownloadRequest, background_tasks: BackgroundTasks) -> dict:
        ...
        if resolve_error:
            # If profile resolution fails, fall back to direct yt-dlp handling.
            resolved_urls = None
